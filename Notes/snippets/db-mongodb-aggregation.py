...
def calculate_document_frequency(
    ...
    - Time-range filtering using $match.
    ...
    if timestamp_field and (start_ts or end_ts):
        match_filter = {}
        if start_ts:
            match_filter['$gte'] = start_ts
        if end_ts:
            match_filter['$lt'] = end_ts
        pipeline.append({"$match": {timestamp_field: match_filter}})
    ...
    if world_size > 1:
      ...
      # Insert the partitioning stage at the beginning of the pipeline
      match_stage_index = 0 if pipeline and "$match" in pipeline[0] else -1
      if match_stage_index != -1:
          pipeline[match_stage_index]["$match"]["$expr"] = partition_expr
      else:
          pipeline.insert(0, {"$match": {"$expr": partition_expr}})
...
def calculate_term_frequency(
    ...
    if event_type_field and bhv_types:
        match_expr[event_type_field] = {"$in": list(set(bhv_types))}
    if timestamp_field:
        match_expr[timestamp_field] = {"$ne": None, "$exists": True}
        time_filter = {}
        if start_ts:
        ...
        if time_filter:
            match_expr[timestamp_field] = time_filter
