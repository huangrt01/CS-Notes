...
* Double hashing (Zhang et al. , utilizes two independent hash functions to reduce memory

  usage, but still has random collision.

  Learning to hash methods
  (Wang et al., 2017) focus on similarity preserving by training ML-
  based hash functions.

* There have also been works that address
  impression skew through contrastive learning or clustering (Yao
  et al., 2021; Chang et al., 2024); we view these as complementary
  approaches
...
##### A Practitioner’s Handbook [Snap]
