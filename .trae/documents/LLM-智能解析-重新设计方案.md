...
2. OpenClaw 接收消息
   ↓
3. OpenClaw 触发智能解析 Skill（方案二）
   ↓
4. Skill 解析任务并写入 todos.json
   ↓
5. Skill 通过 OpenClaw 消息通知用户
   ↓
6. 用户在 Todos Web Manager 中查看和管理任务（方案三）
   ├─ Todos Web Manager 提供语音输入按钮
   ├─ 用户点击按钮开始录音
   ├─ 语音转写为文字
   ├─ 调用 LLM 智能解析
   ├─ 解析结果显示在界面上，用户可以编辑
...
- **todo-20260219-038**: 实现 LLM 优先的智能解析流程（可选，作为增强功能）
...
5. ⏸️ 实现 LLM 优先的智能解析流程（Todo 1，可选）
